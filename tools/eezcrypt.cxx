...
static uint32_t encrypt_f(const uint32_t S[4][256], uint32_t x)
{
    const uint32_t h = S[0][x >> 24] + S[1][x >> 16 & 0xff];
    return (h ^ S[2][x >> 8 & 0xff]) + S[3][x & 0xff];
}

static void encrypt_v(const uint32_t P[32], const uint32_t S[4][256], uint32_t *L, uint32_t *R)
{
...
static void decrypt_v(const uint32_t P[32], const uint32_t S[4][256], uint32_t *L, uint32_t *R)
{
...
static const uint32_t bf_sbox[4][256] = {
    {
...
static const uint32_t bf_parray[32] = {
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
    0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
    0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1bL
};

/// @brief Generate the Blowfish P-array and S-box from the given key
/// @param parray Pointer to the 32-bit array of 32 elements for the final P-array
...
{
    // Copy initial permutations into the P-array and S-box
    memcpy(parray, bf_parray, sizeof(uint32_t) * 32);
    memcpy(sbox, bf_sbox, sizeof(uint32_t) * 4 * 256);

    // Initialize the P box with the key
    for(size_t i = 0, p = 0; i < 18; i++) {
        uint32_t k = 0;
        for(size_t j = 0; j < 4; j++) {
            k = (k << 8) | ((const uint8_t *)key)[p];
            p = (p + 1) % keylen;
        }
        parray[i] ^= k;
    }
...
/// @brief Encrypt a bitstream using the Blowfish algorithm
/// @param _out 
/// @param parray
/// @param sbox
...
void encrypt(void *_out, const uint32_t parray[32], const uint32_t sbox[4][256], const void *_data, size_t len)
{
    ...
    auto *out = reinterpret_cast<uint64_t *>(_out);
    // Encrypt in 8-byte bulks
...
/// @brief Decrypt a Blowfish-encrypted bitstream
/// @param _out 
/// @param parray 
/// @param sbox 
/// @param _data 
...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        ...
        char decryptBuf[msglen + 1];

        genkey(parray, sbox, (const void *)key, keylen);
        std::cout << "Message: '" << msg << "', Key: '" << key << "'" << std::endl;

        encrypt((void *)encryptBuf, parray, sbox, (const void*)msg, msglen);
        encryptBuf[msglen] = '\0';
        std::cout << "Encrypted: '" << encryptBuf << "'" << std::endl;

        decrypt((void *)decryptBuf, parray, sbox, (const void*)encryptBuf, msglen);
        decryptBuf[msglen] = '\0';
